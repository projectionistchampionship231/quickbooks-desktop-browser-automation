/**
 * anchor-cloud.js — AnchorBrowser cloud example for QuickBooks Desktop
 *
...
const { login_quickbooks, create_invoice } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting QuickBooks Desktop automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_quickbooks(page, {});
      log('Action 1 result:', data1);

      const data2 = await create_invoice(page, {});
